...
\begin{titlepage}
    ...
    \textbf{«Київський політехнічний інститут імені Ігоря Сікорського»}\\
    \textbf{Кафедра обчислювальної техніки ФІОТ}

    \vspace{4cm}

    \textbf{\Large ЗВІТ}\\
    \textbf{з лабораторної роботи №1}\\
    \vspace{0.5cm}
    ...
    \textbf{Тема:}\\
    \textbf{\Large БАЗОВІ ГЕОМЕТРИЧНІ ПЕРЕТВОРЕННЯ 2D ТА 3D ОБ’ЄКТІВ}

    \vspace{3cm}

    \begin{flushright}
        \textbf{Виконав:}\\
        Студент курсу \studentcourse{}, групи \studentgroup{}
...
Вектор координат точки: $P = [x, y, 1]^T$. Матриця перетворення має розмірність $3 \times 3$.
...
\subsection{Реалізація логіки Task 2 (notebooks/task2.py)}
Фрагмент коду, що відповідає за динаміку сцени, випадкову зміну параметрів та
візуалізацію.
